<div class="container">
  ...
  <div class="row">
    <div class="col-12 bg-light p-5 rounded">
      ...
      <br />
      <form class="col-md-12 form-inline" action="/inventaris-obat/rekam/custom/detail_rekam/rekam_tgl" method="post">
       ...
       <div class="input-daterange col-md-5 form-inline">
        <div class="col-md-6">
         <input type="text" name="start_date" id="start_date" class="form-control mr-4" autocomplete="off" required/>
        </div>
        <div class="col-md-6">
         <input type="text" name="end_date" id="end_date" class="form-control mr-4" autocomplete="off" required/>
        </div>
        </div>
        <div class="col-md-1">
      ...
      <br />
      <table class="table table-striped display" id="trekam">
        <thead>
          <tr>
            <!-- <th>Kode</th> -->
            <th>#</th>
        ...
        <tbody id="show_data">
          <?php $i=1; foreach ($rekam as $r) {?>
            <tr style="font-weight:normal;">
              ...
              <th><?=$r->rekam_tgl?></th>
              <th>
                <button onclick="tampilObat(this.id)" id="tobat<?=$i?>" data-obat='<?php echo json_encode($controller->getObat($r->rekam_id))?>' class="btn btn-info btn-sm">Lihat Obat</button>
                <button onclick='confirmDelete("<?=base_url()?>rekam/delete/<?=$r->rekam_id?>")' class="btn btn-danger btn-sm">Hapus</button>
              </th>
            </tr>
          <?php $i++; } ?>
          </tbody>
